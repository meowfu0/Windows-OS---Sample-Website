...
function toggleEdgeCard(cardNumber) {
    
    var cardId = 'edgeCard' + cardNumber;
...
function hideEdgeCard(cardNumber) {
    var cardId = 'edgeCard' + cardNumber;
    var cardToHide = document.getElementById(cardId);
...
function toggleEdgeCard(cardNumber) {
    ...
    var allCardContents = card.getElementsByClassName('card-content');

   
    if (card.style.display === 'none') {
      
        card.style.display = 'block';
        Array.from(allCardContents).forEach((content, index) => {
...
function showCardPage(cardNumber, page) {

    var contents = document.querySelectorAll(`#edgeCard${cardNumber} .card-content`);
    contents.forEach(function(content) {
        content.style.display = 'none';
    });

    
    var pageToShow = document.getElementById(`card${cardNumber}-content${page}`);
    if (pageToShow) {
        pageToShow.style.display = 'block';
    } else {
        console.error('Requested content page not found');
    }
}
function hideEdgeCard(cardNumber) {
    var cardId = 'edgeCard' + cardNumber;
...
function hideEdgeCard(cardNumber) {
    ...
    var card = document.getElementById('edgeCard' + cardNumber);
    card.style.display = 'none';

   
    var firstContent = document.getElementById(`card${cardNumber}-content1`);
    var allCardContents = card.getElementsByClassName('card-content');

    Array.from(allCardContents).forEach(content => {
        content.style.display = 'none';
    });
